##########################
# Activation Maximization Algorithm to determine the receptive field of a neuron
# This script was adapted from the following source: https://github.com/Nguyen-Hoa/Activation-Maximization/blob/master/act_max_util.py
# The algorithm is based on Yosinski, Jason, et al. "Understanding neural networks through deep visualization." arXiv preprint arXiv:1506.06579 (2015).
##########################
...
def norm_crop(img, threshold=0):
    ...
    smalls = tile(smalls, (1, 1, 1))  # (3, 1, 1)
...
def act_max(network,
    ...
    for k in range(steps):
        ...
        layer_out[0][0][unit[0]][unit[1]].backward(retain_graph=True)
        img_grad = input.grad
        ...
        with torch.no_grad():
            ...
            if Gaussian_Blur and k % theta_every == 0: #previously  is instead of ==
        ...
        if verbose and k % 25 == 0:
            print('\tstep: ', k, 'activation: ', layer_out[0][0][unit[0]][unit[1]])  # layer_out[0][unit]
        ...
        # print(layer_out[0][0][unit[0]][unit[1]])
        if best_activation < layer_out[0][0][unit[0]][unit[1]]:
            best_activation = layer_out[0][0][unit[0]][unit[1]]
            best_img = input

        if clip_image:
            best_img = best_img.clip(0, 1)
